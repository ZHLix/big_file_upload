...
export default class CourseClass extends base {
        ...
        return this.result({
            ...
            form: {
                ...
                field: {
                    ...
                    // time: {
                    //     label: '课时长', type: 'time', editable: true, format: 'HH:mm:ss', valueFormat: 'HH:mm:ss',
                    //     value: '00:00:00',
    ...
    async list (req) {
        let { page, limit } = req
        let { data: { count, rows: data }, code, msg } = await this.$root.$request.post('api/smallCourse', {
            ...this.params(req),
        ...
        return this.result({ table: data, form: {}, pagination: { total: count, page, limit } }, code, msg)
    }

    // create (req) {
    //     req.time = this.$vm.$utils.str2Seconds(req.time)
    //     return this.$root.$request.post('api/smallCourse/create', { info: Object.assign(req, { course_id: this.course_id }) })
    // }
    ...
    edit (req, id, row) {
        ...
        return this.$root.$request.post('api/smallCourse/edit', { ...req, id, course_id: this.course_id })
    }

    // async del ({ id }) {
    //     return this.$root.$request.post('api/smallCourse/del', { id: id })
    // }
