  ...
  render(h) {
    const item = arr => {
      return arr.map(v => {
        ...
        if (v.sub) {
          elemName += '-group'
          tmp.push(h(elemName, {props: {label: v.name}}, item(v.sub)))
        } else {
          tmp.push(h(elemName, {props: {label: v.name, value: v.value}}))
        }
        return tmp
      })
    }
      ...
      'el-select',
      {
        ...
        on: {
          input: v => {
            ...
            if (this.config.multiple && !(this.value instanceof Array)) {
              v = v.join(',')
            }
            this.$emit('input', v)
          }
